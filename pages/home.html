<!DOCTYPE html>
<html lang="fr">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">

    <link rel="stylesheet" href="../assets/css/style.css">

    <title>WebBook - Home</title>
</head>
<body>
    <div class="container">
        <header>
            <div class="header-left">                
                <div class="logo">
                    <img src="../assets/icon/book_icon.svg"/>
                </div>
                <div class="title">WebBooks</div>
            </div>
            <nav class="menu">
                <a href="#">Home</a>
                <a href="#">Contact</a>
                <a href="#">Search</a>
                <a id="person" href="#"><img src="../assets/icon/person_black_48dp.svg"></a>
            </nav>
        </header>

        <div class="img-back">
            <img src="../assets/img/femme-hamac-livre.jpg"/>
        </div>

        <div class="recent-books">
            <p class="title-recent">Recent books</p>
            <div class="line-recent"></div>
            <div class="list-recent">
                <div class="book-recent">                    
                    <img src="../assets/img/L-enfant-parfaite.jpg">
                    <h2 class="title-book-recent">L'enfant parfaite</h2>
                </div>
                <div class="book-recent">                    
                    <img src="../assets/img/Belle-Greene.jpg">
                    <h2 class="title-book-recent">Belle Greene</h2>
                </div>
                <div class="book-recent">                    
                    <img src="../assets/img/Des-diables-et-des-saints.jpg">
                    <h2 class="title-book-recent">Des diables et des saints</h2>
                </div>
                <div class="book-recent">                    
                    <img src="../assets/img/Le-paeur.jpg">
                    <h2 class="title-book-recent">Le passeur</h2>
                </div>
                <div class="book-recent">                    
                    <img src="../assets/img/Les-Vilaines.jpg">
                    <h2 class="title-book-recent">Les vilaines</h2>
                </div>
                <div class="book-recent">                    
                    <img src="../assets/img/L-ami.jpg">
                    <h2 class="title-book-recent">L'ami</h2>
                </div>
            </div>
        </div>

        <div id="modal-search" class="modal">
            <div>
                <form>
                    <input type="text" />
                </form>
                <div class="search-list">
                    <img src="../assets/img/Fables-Livres-VII-a-XI-oeuvre-au-programme.jpg">
                    <div>
                        <p class="search-title">
                            Fables de La Fontaine, Livres VII à XI (Bac 2021)
                        </p>
                        <p class="search-editor">Hatier</p>
                        <p class="search-date">28/08/2019</p>
                    </div>
                </div>
                <div class="search-list">
                    <img src="../assets/img/Fables.jpg">
                    <div>
                        <p class="search-title">
                            Fables
                        </p>
                        <p class="search-editor">Gallimard</p>
                        <p class="search-date">15/04/2021</p>
                    </div>
                </div>
                <div class="search-list">
                    <img src="../assets/img/A-la-claire-fontaine.jpg">
                    <div>
                        <p class="search-title">
                            Fables (livres VII à XI) - Nouvelle
                        </p>
                        <p class="search-editor">Belin Education</p>
                        <p class="search-date">15/06/2020</p>
                    </div>
                </div>
                <div class="search-list">
                    <img src="../assets/img/Fables-Livres-VII-a-XI-oeuvre-au-programme.jpg">
                    <div>
                        <p class="search-title">
                            Les Fables, Livres VII à XI 
                        </p>
                        <p class="search-editor">Nathan</p>
                        <p class="search-date">02/01/2020</p>
                    </div>
                </div>
                <div class="search-list">
                    <img src="../assets/img/A-la-claire-fontaine.jpg">
                    <div>
                        <p class="search-title">
                            Agatha Raisin - Méfiez-vous de l'eau qui dort! ...
                        </p>
                        <p class="search-editor">Albin Michel</p>
                        <p class="search-date">28/08/2019</p>
                    </div>
                </div>
                <div class="search-list">
                    <img src="../assets/img/Fables-Livres-VII-a-XI-oeuvre-au-programme.jpg">
                    <div>
                        <p class="search-title">
                            Fables de La Fontaine, Livres VII à XI (Bac 2021)
                        </p>
                        <p class="search-editor">Hatier</p>
                        <p class="search-date">28/08/2019</p>
                    </div>
                </div>
            </div>
        </div>

        <div id="modal-sign-in" class="modal">
            <div>
                <form>
                    <label>Email</label>
                    <input type="email" />
                    <label>Password</label>
                    <input type="password" />
                </form>
                <p>Sign out</p>
                <a href="#">Sign in</a>
            </div>
        </div>

        <div id="modal-sign-up" class="modal">
            <div>
                <form>
                    <label>Full Name</label>
                    <input type="text" />
                    <label>Email</label>
                    <input type="email" />
                    <label>Password</label>
                    <input type="password" />
                    <label>Password</label>
                    <input type="password" />
                </form>
                <p>Sign in</p>
                <a href="#">Sign up</a>
            </div>
        </div>
    </div>
</body>
</html>